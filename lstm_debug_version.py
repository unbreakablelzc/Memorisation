...
trainset = Memlog_1(n_samples=486, max_seq_len=seq_max_len)
testset = Memlog_1(n_samples=162, max_seq_len=seq_max_len)
...
with tf.Session() as sess:
    ...
    for step in range(1, training_steps + 1):
        ...
        debug_sess.run(optimizer, feed_dict={x: batch_x, y: batch_y,
                                       seqlen: batch_seqlen})
        if step % display_step == 0 or step == 1:
            # Calculate batch accuracy & loss
            acc, loss = debug_sess.run([accuracy, cost], feed_dict={x: batch_x, y: batch_y,
                                                              seqlen: batch_seqlen})
    ...
    print("Testing Accuracy:", \
          debug_sess.run(accuracy, feed_dict={x: test_data, y: test_label,
                                        seqlen: test_seqlen}))
