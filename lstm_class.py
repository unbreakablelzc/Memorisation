...
#dir_instances = 'data/data_200_200_hard.txt'
    ...
    def __init__(self, max_seq_len, path=""):
        ...
        for line in f:
            ...
            if i % 2 == 0:
                ...
                tj = data_start_time[k]  # the starting time of each job in the sequence
                # [ [tj, pj, dj ],...]
                ...
                #data_tmp = data_tmp + [0 for i in range(max_seq_len - 2 * length - 1 - 400)]
        ...
        self.data = np.array(data_raw).reshape(-1, 1000, 1)
...
class ModelRNN:
    def dynamic_rnn(self, x, seqlen, weights, biases):
        ...
        x = tf.unstack(value=x, num=seq_max_len * 3 + 400, axis=1)
        ...
        with tf.name_scope("LSTM_layers"):
            with tf.name_scope("LSTM_cell"):
                ...
                # calculation.
            outputs, states = tf.contrib.rnn.static_rnn(lstm_cell, x, dtype=tf.float32, sequence_length=seqlen*3)
...
if __name__ == "__main__":
    ...
    with tf.name_scope('input'):
        # tf Graph input
        x = tf.placeholder(tf.float32, [None, seq_max_len*3+400, 1], name="x_input")
